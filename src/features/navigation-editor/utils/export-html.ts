...
/**
 * Escape HTML special characters to prevent XSS in exported HTML files.
 * Used for user-provided text (titles, descriptions) inserted into template literals.
 */
...
export function generateStandaloneHtml(
  categories: Category[],
  structureType: 'current' | 'proposed'
): string {
  // Sort categories alphabetically for the button grid
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* RU Brand Colors */
    :root {
      --ru-red-impact: #e3000b;
      --ru-maroon: #730e04;
      --ru-berry: #8f2011;
      --ru-blue: #008acb;
      --ru-gray: #797777;
      --ru-light-gray: #f8f7f7;
      --ru-mahogany: #4a0004;
      --ru-border: #e0e0e0;
      --ru-text: #303030;
    }
    ...
    .lang-toggle { display: flex; align-items: center; gap: 4px; font-size: 15px; font-weight: 800; }
    .lang-toggle .active { color: var(--ru-berry); text-decoration: underline; }
    .lang-toggle .inactive { color: var(--ru-berry); }
    .lang-toggle .sep { color: #ccc; }
    .search-box { display: flex; align-items: center; border: 1px solid #d7d7d7; overflow: hidden; }
    .search-box input {
      width: 240px;
    ...
    .search-box button:hover { color: var(--ru-red-impact); }
    ...
    .footer-col1-group { min-width: 140px; }
    .footer-col2 {
      width: 260px;
      flex-shrink: 0;
    }
    .footer h3, .footer .footer-col-heading {
      font-size: 18px;
      font-weight: 800;
      color: #730e04;
      margin-bottom: 15px;
    ...
    .social-icons a {
      ...
      height: 30px;
      border-radius: 50%;
      border: 1px solid var(--ru-red-impact);
      background: transparent;
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    /* Accordion Sections */
    .accordion-item { border-top: 1px solid #d7d7d7; }
    .accordion-item:first-child { border-top: 3px solid #730E04; }
    .accordion-header {
      display: flex;
      align-items: center;
      justify-content: space-between;
    ...
    .accordion-header:hover .accordion-title { color: var(--ru-red-impact); }
    .accordion-title {
      font-size: 26px;
      font-weight: 800;
      color: var(--ru-maroon);
    ...
    .accordion-item.expanded .accordion-title { color: var(--ru-red-impact); text-decoration: underline; }
    .accordion-item.expanded .accordion-arrow { color: var(--ru-red-impact); transform: rotate(180deg); }
    .accordion-body {
      display: grid;
      grid-template-rows: 0fr;
      transition: grid-template-rows 0.2s ease-in-out;
    }
    .accordion-item.expanded .accordion-body { grid-template-rows: 1fr; }
    .accordion-body-inner { overflow: hidden; }
    .accordion-body-content { padding: 10px 20px 40px 0; }

    /* Inline Accordion (TipTap Details extension output) */
    .ru-accordion {
      border-top: 1px solid #d7d7d7;
      margin: 1rem 0;
    }
    .ru-accordion:first-of-type {
      border-top: 3px solid #730E04;
    }
    .ru-accordion-title {
      font-weight: 800;
      color: var(--ru-maroon);
    ...
    .ru-accordion-title::-webkit-details-marker {
      display: none;
    }
    .ru-accordion-title::after {
      content: '↓';
      color: var(--ru-maroon);
      font-size: 16px;
      flex-shrink: 0;
    ...
    details.ru-accordion[open] .ru-accordion-title {
      color: var(--ru-red-impact);
      text-decoration: underline;
    }
    details.ru-accordion[open] .ru-accordion-title::after {
      color: var(--ru-red-impact);
      transform: rotate(180deg);
    }
    .ru-accordion-content {
      padding: 10px 20px 40px 0;
    }

    /* Rich-text content styles (scraped HTML) */
    ...
    .page-content h2, .section-content h2 {
      font-size: 26px;
      font-weight: 800;
      color: #730E04;
      letter-spacing: -1px;
    ...
    .page-content a, .section-content a {
      color: #E3000B;
      text-decoration: underline;
    }
    .page-content a:hover, .section-content a:hover {
      color: #730E04;
    }
    .page-content ul, .section-content ul {
      list-style: none;
      padding-left: 0;
      margin: 0 0 18px;
    }
    .page-content ul li, .section-content ul li {
      position: relative;
      padding-left: 20px;
      margin-bottom: 4px;
    }
    .page-content ul li::before, .section-content ul li::before {
      content: '';
      position: absolute;
      left: 0;
      top: 11px;
    ...
    .page-content ol, .section-content ol {
      list-style: none;
      counter-reset: ol-counter;
      padding-left: 0;
      margin: 0 0 18px;
    }
    .page-content ol li, .section-content ol li {
      counter-increment: ol-counter;
      position: relative;
      padding-left: 35px;
      margin-bottom: 4px;
    }
    .page-content ol li::before, .section-content ol li::before {
    ...
    .page-content table, .section-content table {
      width: 100%;
      border-collapse: collapse;
      margin: 0 0 18px;
    }
    .page-content table th, .section-content table th {
      border-bottom: 2px solid #730E04;
      color: #730E04;
      font-weight: 700;
      text-align: left;
      padding: 10px 15px;
    }
    .page-content table td, .section-content table td {
      border-bottom: 1px solid #d7d7d7;
      padding: 10px 15px;
    }
    .page-content strong, .section-content strong {
      font-weight: 700;
    }
...
<body>
  <div class="page-wrapper">
    ...
    <div class="content-wrapper">
      <div class="main-layout">
        ...
        <main class="main-content">
          ...
          <!-- Category Views -->
          ${categories.map(cat => `
          <div id="view-category-${cat.id}" class="hidden">
            ...
            ` : `
            <div class="page-sections" style="margin-top: 40px;">
              ${cat.sections.map(section => `
          ...
          <!-- Page Detail Views -->
          ${categories.map(cat => (cat.pages || []).map(page => `
          <div id="view-page-${page.id}" class="hidden">
            ...
            ${!page.intro && !page.description && !page.content && (!page.sections || page.sections.length === 0) ?
              `<p style="color: var(--ru-gray); font-style: italic;">Geen gedetailleerde inhoud beschikbaar voor deze pagina.</p>` : ''}
            <div style="margin-top: 24px;">
              <button onclick="showCategory('${cat.id}')" style="color: var(--ru-red-impact); font-size: 14px;">
                ← Terug naar ${escapeHtml(cat.label)}
              </button>
            </div>
          </div>
          `).join('')).join('')}
        ...
        <div class="footer-col1">
          ...
          <div>
            ...
            <ul>
              ...
              <li><a href="#">Faciliteiten &amp; samenwerken</a></li>
              <li><a href="#">Werken bij</a></li>
              <li><a href="#">De Radboud Universiteit</a></li>
            </ul>
          </div>
          <div>
            <h3>Contact</h3>
            <ul>
              ...
              <li><a href="#">Zoek medewerker</a></li>
    ...
    <div class="footer-secondary">
      ...
      <div class="footer-logo-block">
        <svg class="footer-logo" viewBox="0 0 198 42" xmlns="http://www.w3.org/2000/svg"><path fill="#222" d="M55.33 5.72h12.54c4.14 0 7.18 1.04 9.14 3.1 1.62 1.72 2.44 3.88 2.44 6.5v.08c0 2.28-.56 4.14-1.68 5.6-1.12 1.46-2.62 2.54-4.48 3.26l7.04 10.28H73.5l-6.22-9.26h-4.92v9.26h-7.03V5.72Zm12.14 13.7c1.48 0 2.62-.36 3.42-1.08.8-.72 1.2-1.66 1.2-2.82v-.08c0-1.28-.42-2.24-1.26-2.9-.84-.66-2-.98-3.46-.98h-4.98v7.86h5.08Zm16.58-13.7h23.42v6.04H91.12v4.76h14.84v5.62H91.12v4.96h16.55v6.04H83.97V5.72h.08Zm27.5 0h7.3l11.64 15.26V5.72h6.96v28.82h-6.74l-12.2-15.98v15.98h-6.96V5.72Zm30.5 0h7.02v28.82h-7.02V5.72Zm11.82 0h7.72l6.04 19.34 6.22-19.34h6.26l6.22 19.34 6.04-19.34h7.58l-10.1 29.06h-6.34l-6.3-18.96-6.3 18.96h-6.34L153.97 5.72Zm48.42-.24h6.66l13.48 29.3h-7.44l-2.74-6.22h-13.36l-2.74 6.22h-7.26l13.4-29.3Zm7.44 17.22-4.14-9.42-4.14 9.42h8.28Z"/><path fill="#BE311A" d="M21 3.93a6.988 6.988 0 0 1 2.695 1.97c.95.29 1.307 1.04 1.307 1.75v2.574h2.797l.133.378c.346 1.027.477 2.15.477 3.374 0 3.786-1.395 8.216-2.777 11.672-2.082 5.218-4.852 9.946-6.318 11.52-.333.342-.612.64-1.08.66-.009.004-.017 0-.03 0-.008 0-.022.004-.032 0-.451-.02-.726-.318-1.068-.66-1.43-1.574-4.178-6.302-6.256-11.52C9.478 21.786 8.076 17.356 8.076 13.57c0-1.224.15-2.347.487-3.374l.12-.378h2.787V7.65c0-.71.36-1.46 1.297-1.751.663-.834 1.62-1.529 2.68-1.97V1.58h1.571V0h2.379v1.58h1.604V3.93Z"/></svg>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function updateSidebarActive(categoryId) {
      document.querySelectorAll('.sidebar-subnav button').forEach(btn => {
        btn.classList.remove('active');
      });
      if (categoryId) {
        const btn = document.getElementById('sidebar-' + categoryId);
 ...
 */
export function downloadHtml(categories: Category[], structureType: 'current' | 'proposed'): void {
  const html = generateStandaloneHtml(categories, structureType);
  const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
  const url = URL.createObjectURL(blob);

  const link = document.createElement('a');
  link.href = url;
  link.download = `ict-preview-${structureType}-${new Date().toISOString().split('T')[0]}.html`;
